...
The file containing the private key to request a certificate for via
a IR, CR or KUR.  Default is the current client key if given.
The client inserts the corresponding public key in the request.
On receiving the newly enrolled certificate it checks that
the included public key matches the private key used for the request.
...
For P10CR the essence of the request is supplied via this option, while the
above certificate request options (B<-newkey>, ..., B<-popo>) are ignored.
