...
S<[B<-revcheck_tls spec>]>
S<[B<-revcheck_cmp spec>]>
S<[B<-revcheck_new spec>]>
S<[B<-crls URLs>]>
S<[B<-disable_cdps>]>
S<[B<-cdp url>]>
S<[B<-crl_timeout seconds>]>
S<[B<-disable_aias>]>
S<[B<-ocsp_url url>]>
...
=head2 Specific certificate verification options

=over 4

=item B<-revcheck_tlsE<nbsp>(none|leaf|any|full)[:(crls|ocsp|ocsp_last|stapling)]>

Specify range and methods of TLS server certificate revocation status checks.
For details see below.

=item B<-revcheck_cmp (none|leaf|any|full)[:(crls|ocsp|ocsp_last)]>

Specify range and methods of CMP server certificate revocation status checks.
For details see below.

=item B<-revcheck_new (none|leaf|any|full)[:(crls|ocsp|ocsp_last)]>

Specify range and methods of revocation status checks on newly enrolled
certificate.
...
=item * B<none>: no revocation checks done, which is the default

=item * B<leaf>: check revocation of first certificate in chain

=item * B<any>: check revocation of all certificates having a CDP or AIA entry

=item * B<full>: check revocation of whole chain

=back

The range specification is optionally followed by a method selector.
The default is to try OCSP stapling first (only for TLS), then OCSP, then CRLs.
Possible values are:

=over 2

=item * B<stapling>: try status check for TLS server certificate
via OCSP stapling with plain OCSP as fallback.
This option is available only for the TLS level
and with OpenSSL version 1.1.0 or newer. Multi-stapling is not (yet) supported).

=item B<ocsp>: use OCSP only

=item B<crls>: use CRLs only

=item B<ocsp_last>: try OCSP stapling (for TLS), then CRLs, then OCSP

=back

=item B<-crls URLs>

Pre-load the given CRL(s) as local primary source of certificate revocation
information (to be used right after any stapled OCSP responses, if applicable).
Multiple sources may be specified, separated by commas or whitespace,
each starting with C<http:> or C<file:> or being simply a local file name.

=item B<-disable_cdps>

Disable use of CDP entries in certificates.

=item B<-cdp url>

Fallback URL for CRL Distribution Point (CDP);
CDP entries in certificates are preferred.

=item B<-crl_timeout seconds>

Timeout for online CRL retrieval via HTTP (or 0 for none).
Default is 10 seconds.

=item B<-disable_aias>

Disable use of AIA entries in certificates.

=item B<-ocsp_url url>

Fallback URL for OCSP responder (AIA entries in certificates are preferred).
...
=head2 Standard certificate verification options

=over 4

=item B<-crl_check> and B<-crl_check_all>

These two are considered legacy here because they are superseded by the above
B<-revcheck_...> options. If still used, they require CRL checks at all levels:
TLS (if applicable), CMP, and for newly enrolled certs (if applicable).
...
L<genrsa(1)|genrsa(1)>, L<list(1)|list(1)>, L<x509v3_config(5)|x509(1)>
