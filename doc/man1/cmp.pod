...
S<[B<-certpass arg>]>
S<[B<-certform DER|PEM|P12>]>
S<[B<-keyform DER|PEM|P12>]>
S<[B<-clrform DER|PEM>]>
S<[B<-storeform DER|PEM|P12>]>
S<[B<-engine id>]>
...
and/or the issure of the newly enrolled certificate
(when verifying a certificate response).
...
if no sender name can be determined from the B<-cert> or <-subject> options
and is optional when authenticating with pre-shared key (password-based MAC).
...
Flag the Subject Alternative Name(s) given via B<-san_dns> or B<-san_ip>
as critical.

=item B<-san_nodefault>

When Subject Alternative Names are not given via B<-san_dns> or B<-san_ip>
nor defined via B<-reqexts>,
they are are copied by default from the reference certificate (see B<-oldcert>).
...
Set Proof-of-Possession (POPO) method; values: C<0>..<3> where
C<0> = NONE, C<1> = SIGNATURE (default), C<2> = ENCRCERT, C<3> = RAVERIFIED.
