...
S<[B<-server> I<address[:port]>]>
S<[B<-proxy> I<address[:port]>]>
S<[B<-path> I<remote_path>]>
S<[B<-msgtimeout> I<seconds>]>
S<[B<-totaltimeout> I<seconds>]>

S<[B<-trusted> I<filenames>]>
S<[B<-untrusted> I<sources>]>
S<[B<-srvcert> I<filename>]>
S<[B<-recipient> I<name>]>
S<[B<-expect_sender> I<name>]>
S<[B<-ignore_keyusage>]>
S<[B<-unprotectederrors>]>
S<[B<-extracertsout> I<filename>]>
S<[B<-cacertsout> I<filename>]>
...
S<[B<-digest> I<name>]>
S<[B<-mac> I<name>]>
S<[B<-extracerts> I<sources>]>
S<[B<-unprotectedrequests>]>

S<[B<-cmd> I<ir|cr|kur|p10cr|rr|genm>]>
S<[B<-infotype> I<name>]>
S<[B<-geninfo> I<OID:int:N>]>

S<[B<-newkey> I<filename>]>
S<[B<-newkeypass> I<arg>]>
S<[B<-subject> I<name>]>
S<[B<-issuer> I<name>]>
S<[B<-days> I<number>]>
S<[B<-reqexts> I<name>]>
S<[B<-sans> I<spec>]>
S<[B<-san_nodefault>]>
S<[B<-policies> I<names>]>
S<[B<-policies_critical>]>
S<[B<-popo> I<number>]>
S<[B<-csr> I<filename>]>
S<[B<-out_trusted> I<filenames>]>
S<[B<-verify_hostname> I<cn>]>
S<[B<-verify_ip> I<ip>]>
S<[B<-verify_email> I<email>]>
S<[B<-implicitconfirm>]>
S<[B<-disableconfirm>]>
S<[B<-certout> I<filename>]>

S<[B<-oldcert> I<filename>]>
S<[B<-revreason> I<number>]>

S<[B<-ownform> I<PEM|DER|P12>]>
S<[B<-keyform> I<PEM|DER|P12>]>
S<[B<-crlform> I<PEM|DER>]>
S<[B<-otherform> I<PEM|DER|P12>]>
S<[B<-otherpass> I<arg>]>
S<[B<-engine> I<id>]>

S<[B<-tls_used>]>
S<[B<-tls_cert> I<filename>]>
S<[B<-tls_key> I<filename>]>
S<[B<-tls_keypass> I<arg>]>
S<[B<-tls_extra> I<filenames>]>
S<[B<-tls_trusted> I<filenames>]>
S<[B<-tls_host> I<name>]>
...
S<[B<-ocsp_use_aia>]>
S<[B<-ocsp_url> I<url>]>
S<[B<-ocsp_timeout> I<seconds>]>
S<[B<-ocsp_status>]>
S<[B<-policy> I<arg>]>
S<[B<-purpose> I<purpose>]>
S<[B<-verify_name> I<name>]>
S<[B<-verify_depth> I<num>]>
S<[B<-auth_level> I<level>]>
S<[B<-attime> I<timestamp>]>
S<[B<-ignore_critical>]>
S<[B<-issuer_checks>]>
S<[B<-policy_check>]>
S<[B<-explicit_policy>]>
...
S<[B<-poll_count>]>
S<[B<-checkafter>]>
S<[B<-grant_implicitconf>]>
S<[B<-pkistatus>]>
S<[B<-failure>]>
S<[B<-failurebits>]>
S<[B<-statusstring>]>
S<[B<-send_error>]>
S<[B<-send_unprotected>]>
S<[B<-send_unprot_err>]>
S<[B<-accept_unprotected>]>
S<[B<-accept_unprot_err>]>
S<[B<-accept_raverified>]>
...
Default filename is taken from the environment variable C<OPENSSL_CONF>.
...
The IP address or hostname and port (defaults to 80)
of the CMP server to connect to using HTTP/S transport.

=item B<-proxy> I<address[:port]>

The IP address or hostname and port (defaults to 80) of the HTTP proxy server,
...
Multiple filenames may be given, separated by commas or whitespace.
...
B<-expect_sender> I<name> will continue to match after updates of the server cert.
...
e.g., hmacWithSHA256. Defaults to C<hmac-sha1> as per RFC 4210.

=item B<-extracerts> I<sources>

Certificates to append in the extraCerts field when sending messages.

Multiple filenames or URLs may be given, separated by commas or whitespace.
Each source may contain multiple certificates.
...
in IR, CR or KUR.  Default is the public key in the PKCS#10 CSR given with the
B<-csr> option, if any, or else the current client key, if given.
...
Set Proof-of-Possession (POPO) method in IR/CR/KUR; values: C<-1>..<2> where
...
=item B<-otherform> I<PEM|DER|P12>
...
Address to be checked (rather than B<-server> address) during hostname
...
each element starting with C<http:> or C<file:> or being a file (path) name.

This option enables CRL checking for, e.g., the CMP/TLS server certificate
as it is the leaf certificate of the certificate chain.
Compare B<X509_V_FLAG_CRL_CHECK>.

=item B<-crl_timeout> I<seconds>

Timeout for online retrieval of CRL from CRL distribution points (CDS)
or 0 for infinite. Default is 10 seconds.
...
Timeout for retrieving responses from OCSP responders or 0 for infinite.
...
B<-auth_level>, B<-attime>, B<-ignore_critical>, B<-issuer_checks>,
B<-policy_check>,
...
=head2 Mock server options, used for test purposes only
...
Reference value to use as senderKID of server in case no B<-cert> is given.
...
Number oftimes the client must poll before receiving a certificate.
...
This prints information about all received ITAV B<infoType>s to stdout.
  ...
  openssl cmp -cmd genm -server 127.0.0.1:80 -path pkix/ \
    -ref 1234 -secret pass:1234-5678-1234-5678 \
    -recipient "/CN=CMPserver"
