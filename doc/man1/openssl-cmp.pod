...
to be use for accessing the CMP server (unless NO_PROXY applies, see below).
Defaults to value of environment variable 'http_proxy', else HTTP_PROXY.

=item B<-no_proxy> I<addresses>
List of addresses of servers not use use HTTP(S) proxy for,
separated by commas or whitespace.
Defaults to value of environment variable 'no_proxy', else NO_PROXY, else none.
...
Maximum number of messages the CMP HTTP server mock-up should handle,
which must be non-negative.
The default value is 0, which means that no limit is imposed.
In any case the mock server terminates when a message processing error occurs.
