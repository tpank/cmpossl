...
openssl-cmp - client for the Certificate Management Protocol (RFC4210)
...
[B<-ownform> I<PEM|DER|P12>]
[B<-keyform> I<PEM|DER|P12>]
[B<-crlform> I<PEM|DER>]
[B<-otherform> I<PEM|DER|P12>]
[B<-otherpass> I<arg>]
[B<-engine> I<id>]
...
reference to the specified engine, initializing it if needed. The engine
...
=head2 Mock server options, used for testing purposes only

=over 4

=item B<-use_mock_srv>

Use the mock server (not a real one) for the client.
...
The CMP HTTP server can serve and respond a specified amount of messages or it
is able to serve all received message until an error occours during request
processing.
This behavior can be controlled by the B<-max_msgs> followed by a non-negative
number.
If the parameter is set to a positive number this given number of CMP messages
will be processes before terminating the CMP HTTP servers process.
If the parameter is set to 0 or omitted the CMP HTTP server will serve all
requests without terminating in between.
But in any case the server will terminate if an message processing error occurs.
