...
=item B<-proxy> I<[http://]address[:port]>

The IP address or DNS hostname and optionally port (defaulting to 80)
of the HTTP(S) proxy server to use for reaching the CMP server
unless no_proxy applies, see below.
Defaults is from the environment variable C<http_proxy>, else C<HTTP_PROXY>,
else none.

=item B<-no_proxy> I<addresses>
List of IP addresses and/or DNS names of servers not use an HTTP(S) proxy for,
separated by commas or whitespace.
Default is from the environment variable C<no_proxy>, else C<NO_PROXY>,
else none.
...
Unless the B<-secret> option indicating PBM is given,
...
Format to try first when reading key files.
Default value is PEM.

=item B<-crlform> I<PEM|DER>

Format to try first when reading CRL files.
Default value is PEM.
...
Multi-stapling is not (yet) supported.
Not available before OpenSSL 1.1.
...
check the protection of the response message.
Yet there are CMP services based on EJBCA,
which tends not to protect negative responses.
In this case
...
on the command line.
Therefore the client also offers the possibility to read
