...
=item B<-server> I<host[:port][/path]>

The IP address or hostname and optional port (defaults to 80)
of the CMP server to connect to using HTTP/S transport.
If this provides a complete URL then its path component is used unless
overridden by B<-path>.
...
Defaults to the path component of the server URL, if present, else "/".
...
Host to be checked (rather than B<-server> host) during hostname
