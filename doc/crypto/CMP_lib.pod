...
=head1 SYNOPSIS
  ...
  char *CMP_CERTREPMESSAGE_PKIFailureInfoString_get0(CMP_CERTREPMESSAGE *crepmsg, long rid);
  ...
  X509 *CMP_CERTREPMESSAGE_get_certificate(CMP_CTX *ctx, CMP_CERTREPMESSAGE *crepmsg, long rid);
...
CMP_calc_protection_pbmac() calculates the PBMAC protection for given B<PKImessage> utilizing the given B<secret>.
...
CMP_CERTREPMESSAGE_PKIFailureInfoString_get0() converts the PKIFailureInfo in
the certResponse with the certReqId B<rid> of the certResponseMessage B<crepmsg> 
into a readable string.
Returns a character array of given certRep message, or NULL on error.
...
CMP_CERTREPMESSAGE_get_certificate() attempts to retrieve the returned
certificate from the certResponse with the certReqId B<rid> of the
certResponseMessage B<crepmsg>.
Takes the newKey in case of indirect Pop from, and sets the "lastPKIStatus" in
B<ctx>.
