...
CMP_PKIMESSAGE_check_received() checks for the given received message (i.e.,
...
its transaction ID matches the one contained in the ctx (if any, else learns the
current value), and its recipNonce matches the senderNonce given (if any).
ctx. If so, learns the new senderNonce contained in the message (if any).
It returns the body type (which is >= 0) of the message on success, -1 on error.

sk_X509_add_certs() adds all or not self-signed certificates to given stack.

X509_STORE_add_certs() adds all or self-signed certificates to given store.

X509_STORE_get1_certs() retrieves a copy of all certificates in the given store.
