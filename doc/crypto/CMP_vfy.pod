...
In the case of signature algorithm the certificate can be provided directly in
...
In case of successfull validation of PBM protection of a certificate response
...
Those could, e.g., apply to later Polling Responses (pollRep)
or PKI Confirmation (PKIConf) messages in the same transaction.

If ctx->permitTAInExtraCertsForIR is true, the trust anchor may be taken from
the extraCerts field when a self-signed certificate is found there that can
