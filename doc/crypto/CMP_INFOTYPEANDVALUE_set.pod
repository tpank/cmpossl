...
=head1 SYNOPSIS
  ...
  CMP_INFOTYPEANDVALUE *CMP_INFOTYPEANDVALUE_new_generic(ASN1_OBJECT *t,
                                                         ASN1_TYPE *v)
...
infoValue of B<value>. This function uses the pointers B<ttype> and B<vlaue>
internally, so they must B<not> be freed up after the call.

CMP_INFOTYPEANDVALUE_stack_item_push0() is an internal function ath pushes
...
Evaluate whether CMP_INFOTYPEANDVALUE_stack_item_push0() should be listed in this
documentation - as it is an internal function.
