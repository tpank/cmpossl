...
CMP_CTX_set_cdp_callback() sets callback function for CRL retrieval
to be used as secondary source during CMP server certificate verification.

CMP_CTX_set_certVerify_callback() sets callback function to be used
during certificate verification for diagnosis, and optionally modifying the result.
