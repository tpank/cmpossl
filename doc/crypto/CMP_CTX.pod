...
proof-of-possession method to POPOSigningKey
...
RFC 4210.
    ...
    CMP_CTX_OPT_POPMETHOD
        Select the proof of possession method to use. Possible values are:
            ...
            CRMF_POPO_RAVERIFIED - assert that the RA has already
                                   verified the PoPo
        See also CMP_CTX_set1_popoMethod() with the same effect.
    ...
    CMP_CTX_SET_SUBJECTALTNAME_CRITICAL
	Set the SubjectAltName field to be critical

    CMP_CTX_OPT_REVOCATION_REASON
        Set the reason code to be included in revocation request (RR);
...
CMP_CTX_geninfo_itav_push0() adds InfoTypeAndValue item given to the GeneralInfo
filed of the CMP PKIMessage header of the request message to be sent.
...
CMP_CTX_set1_popoMethod() sets the proof-of-possession method to be used
for ir, kur etc. Possible values are defined in crmf.h:

=over 8

=item * CRMF_POPO_NONE      - POP field omitted. According to RFC 4211, section 4, CA/RA uses out-of-band method to verify POP. Note that servers may fail in this case, resulting for instance in HTTP error code 500 (Internal error).
...
=item * CRMF_POPO_RAVERIFIED - assertion according to RFC 4211, section 4, that the RA has already verified the POP
...
CMP_CTX_get_tlsBIO() returns the SSL/TLS BIO to be used for HTTPS
...
CMP_CTX_set_digest() sets the digest algorithm NID to be used in RFC 4210's 
MSG_SIG_ALG, if applicable used for message protection and Proof-of-Posession.
