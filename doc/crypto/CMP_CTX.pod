...
RFC 4210.
    ...
    CMP_CTX_OPT_MSGTIMEOUT
        Maximum amount of time, in seconds a CMP message round trip is allowed
        to take before a timeout error is returned. Default is 120.

    CMP_CTX_OPT_MAXPOLLTIME
        Maximum amount of time, in seconds, to attempt polling
        for a response to a message. Default is 0 (infinite).
...
CMP_CTX_set_msg_transport sets the message transport callback function used for
...
int (*cmp_transportfn_t) (const CMP_CTX *ctx, const CMP_PKIMESSAGE *req, CMP_PKIMESSAGE **res);
