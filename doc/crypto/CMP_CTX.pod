...
=head1 SYNOPSIS
 ...
 int CMP_CTX_set1_untrusted_certs( CMP_CTX *ctx, STACK_OF (X509) *store);
...
CMP_CTX_set1_srvCert() sets the server certificate to be used for verifying
response messages. It should be used together with CMP_CTX_set0_trustedStore()
with a trusted store argument that contains this cert.
...
functions to set the "sender" field for outgoing messages.
...
shall be accepted the callback must return -1 (indicating success).
Else a bit number <= CMP_PKIFAILUREINFO_MAX indicates failure, and the text
