...
Use given CRL(s) as primary source of revocation information. URL may point to local file if prefixed by 'file:'.

This option enables CRL checking for the CMP server certificate used for CMP message protection as it is the
...
Enables CRL checking for the entire certificate chain when verifying certificates used for CMP message protection.
Compare B<X509_V_FLAG_CRL_CHECK_ALL>.

This option requires that either the B<-crls> or B<-cdps> option is also set.
