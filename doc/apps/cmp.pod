...
Use given CRL(s) as primary source when verifying CMP certificates. URL may point to local file if prefixed by 'file:'.

=item B<-cdps>

Retrieve CRLs from distribution points given in certificates as secondary source while verifying CMP certificates.
