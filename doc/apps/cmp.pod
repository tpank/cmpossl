...
B<genm> requests "all relevant information" with an empty General Message.  Upon
receipt of the General Response, information about all received ITAV B<infoType>s
is printed to to stdout.
...
=item B<-geninfoint OID:int>

Set generalInfo in request PKIHeader with type and Integer value given in form OID:int, e.g. '1.2.3:987'.

=item B<-engine id>

Specifying an engine B<id> will cause L<cmp(1)> to obtain a functional
...
information about all received ITAV B<infoType>s to stdout.
