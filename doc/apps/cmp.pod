...
Password for client's TLS key.  If not given here, the password will be prompted
for if needed.  For more information about the format of B<arg> see the
...
pre-shared keys (reference value / secret). For more information about the
format of B<arg> see the B<PASS PHRASE ARGUMENTS> section in L<openssl(1)>.
...
password will be prompted for if needed.  For more information about the format
...
The file containing the key corresponding to the requested certificate in IR,
CR or KUR.  Default is the current client key.
...
in the ./capubs.pem file.
  ...
  openssl cmp -cmd ir -server 127.0.0.1:8080 -path pkix/ \
    -user 1234 -pass pass:1234-5678-1234-5678 \
