...
message protection or verifying the newly enrolled certificate, respectively).
...
Possibly further acceptance criteria may be checked by the callback function
that can be set using CMP_CTX_set_certConf_callback().
...
-verify_out option), check if its Subject DN or any Subject Alternative Name
...
=item B<-tls-untrusted filename>

Additional untrusted certificates for chain building verifying TLS certificates.
...
-crl_check, -crl_download and, -crls require certificate status checking for
at least the leaf certificate using CRLs unless OCSP is enabled and succeeds.
