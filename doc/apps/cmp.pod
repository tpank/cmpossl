...
Section to use within config file defining default CMP options,
which may be overridden at the command line. Default is 'cmp'.
...
CMP server certificate to be used for verifying the protection of messages
received and for checking the newly enrolled cert
(a warning is given if it cannot be verfied).
...
as well as for checking the newly enrolled cert, unless the -srvcert option
is given. The file can contain multiple certificates.

=item B<-untrusted filename>

Intermediate certificates that may be useful for path construction in
CMP server authentication. The file can contain multiple certificates.
