...
Number of seconds a CMP message round trip is allowed
to take before a timeout error is returned. Default is 120.
...
If set, the issuer of the certificate is also used as the receiver.
...
=over 4
=item * error messages
=item * certificate responses (IP/CP/KUP) and revocation responses (RP) with
rejection.
=item * PKIConf
...
=item * Exactly specify which other options is influcenced by the B<engine>
option.
...
=item * Message sequences other than those needed for IR/CR/KUR/RR/GENM
(functionality not yet available in the library).
