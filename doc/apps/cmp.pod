...
The IP address or hostname and port of the proxy server, if needed for accessing the CMP server.
...
If the CMP server certificate is given with the -srvcert option,
recipient name is taken from there and the -recipient option is ignored.
If the -recipient option is not given, the recipient name defaults to the
name given in the -issuer option.  As last resort the subject name of the
issuer of the certificate given with the -oldcert option, if any, is used.

If none of the options allowing to derive the recipient name is given, no
suitable value for the recipient in the PKIHeader is available and it is set to
NULL-DN.
...
and for checking the newly enrolled cert (a warning is given if it cannot be verfied).
May be set alternatively to the -trusted option if the certificate is available and should be directly trusted.
...
Trusted certificates used for CMP server authentication and verifying replies,
as well as for checking the newly enrolled cert, unless the -srvcert option is given.
...
Reference value to be used when messages are authenticated with pre-shared keys
(password-based MAC).  Implies that the B<-secret> option is also given.

=item B<-secret arg>

Source for secret value to be used when messages are authenticated with
...
Set generalInfo in request PKIHeader with type OID and integer value N, e.g. '1.2.3:int:987'.

=item B<-digest name>

Specifies name of supported digest to be used in RFC 4210's MSG_SIG_ALG.
If applicable, this is used for message protection and Proof-of-Possession (POPO)
...
=item * section 5.1.3.1 allows exceptions from protecting only for special cases:
...
The file containing the private key corresponding to the requested certificate in IR,
CR or KUR.  Default is the current client key if given.
...
If neither -srvcert, -recipient, nor -issur is available, the name given in this
issuer of oldcert is also set as the recipient of the CMP message.
...
reference to the specified engine, thus initialising it if needed.
The engine will be used for all algorithms supported for keys prefixed by 'engine:'.
