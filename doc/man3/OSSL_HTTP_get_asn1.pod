...
to the given B<host> and optional B<port> and B<path>,
optionally via B<proxy> and B<port>.
The optional list B<headers> may contain additional custom HTTP header lines.
The B<maxline> may be used to give the response header maximum line length,
while the value 0 indicates the default value, which currently is 4096.
...
whereby it may make use of a custom defined argument 'arg',
...
the function is invoked with the B<detail> argument being 1.
On disconnect B<detail> is 0 if no error occurred or else the last error code.
...
It returns the resulting string components via the given addresses of strings
and integer pointers. The strings must be deallocated by the caller using
<OPENSSL_free(3)> unless they are NULL, which is their default value on error.
