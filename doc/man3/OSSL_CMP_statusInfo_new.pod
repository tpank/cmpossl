...
with the given values. It sets the status field to B<status>, statusString to
B<text>, and interprets the B<failInfo> as bit pattern for the failInfo field.
It returns a pointer to the structure on success, or NULL on error.
