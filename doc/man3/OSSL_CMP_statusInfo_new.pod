...
=head1 SYNOPSIS
  ...
  OSSLL_CMP_PKIFAILUREINFO *OSSL_CMP_PKISI_failInfo_get0(OSSL_CMP_PKISI *si);
...
with the given values. It sets the status field to B<status>.  If B<text> is
not NULL, it is copied to statusString.  If B<failInfo> is >= 0 and <
OSSL_CMP_PKIFAILUREINFO_MAX, it is interpeted as bit pattern for the failInfo
field.  Returns a pointer to the structure on success, or NULL on error.
...
with index B<codeBit> in the PKIFailureInfo of the B<si>, or -1 on error.
