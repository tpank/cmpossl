...
=head1 NAME
  ...
  CMP_ITAV_get0_type,
  CMP_ITAV_get0_value,
  CMP_ITAV_stack_item_push0 - CMP_ITAV utility functions

=head1 SYNOPSIS

  #include <openssl/cmp.h>
  void CMP_ITAV_set(CMP_ITAV *itav,
                                const ASN1_OBJECT *type,
                                const ASN1_TYPE *value);
  ASN1_OBJECT *CMP_ITAV_get0_type(CMP_ITAV *itav);
  ASN1_TYPE *CMP_ITAV_get0_value(CMP_ITAV *itav);

  #include "cmp_int.h"
  int CMP_ITAV_stack_item_push0(
                                STACK_OF(CMP_ITAV) **itav_sk_p,
                                const CMP_ITAV *itav);

=head1 DESCRIPTION

CMP_ITAV_set() sets the B<itav> with an infoType of B<type> and an
...
CMP_ITAV_get0_type() returns a direct pointer to the infoType in the
B<itav>.

CMP_ITAV_get0_value() returns a direct pointer to the infoValue in
the B<itav> as generic ASN1_TYPE*.

CMP_ITAV_stack_item_push0() is an internal function that pushes
...
CMP_ITAV_set() does not return a value.

CMP_ITAV_stack_item_push0() returns 1 on success, 0 on error.
...
in the requests' PKIHeader's genInfo field.
    ...
    CMP_ITAV *itav = CMP_ITAV_new();
    CMP_ITAV_set(itav, type, val);
...
Evaluate whether CMP_ITAV_stack_item_push0() should be listed in
