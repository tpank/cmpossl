...
This is the API for doing CMP (Certificate Management Protocol)
client-server message sequences using OpenSSL.
The default conveying protocol is HTTP.

OSSL_CMP_exec_IR_ses() obtains an initial certificate from the CA.
A populated OSSL_CMP_CTX structure is required as an argument.

OSSL_CMP_exec_CR_ses() requests an additional certificate from the CA.
A populated OSSL_CMP_CTX structure is required as an argument.
...
OSSL_CMP_exec_KUR_ses() obtains an updated certificate from the CA.
A populated OSSL_CMP_CTX structure is required as an argument.
