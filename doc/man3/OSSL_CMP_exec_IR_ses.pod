...
OSSL_CMP_try_certreq() is an alternative to these four functions with
more uniformity w.r.t. the certificate request type and
more flexibility w.r.t. what to do after receiving a checkAfter value.
Besides the B<ctx> it takes the type of request (ir/cr/p10cr/kur) as an argument.
When called for the first time (with no certificate request currently in progress
for the given B<ctx>) it starts requesting a new certificate.
Otherwise (i.e., according to B<ctx> a 'waiting' status has been received before)
it continues polling for the pending request
unless the B<req_type> argument is < 0, which aborts the request.
If the requested certificate is available the function returns -1 and
the caller can use B<OSSL_CMP_CTX_get0_newCert()> to retrieve the new certificate.
If no error occurred but no certificate is available yet then
OSSL_CMP_try_certreq() remembers in the CMP context that polling should be retried
and returns a positive 'checkAfter' value indicating the number of seconds
the caller should let pass before trying again.
In this case the caller can decide to sleep for some time and/or do anything else
before retrying by calling OSSL_CMP_try_certreq() again with the same parameter
values as before.
In this case OSSL_CMP_try_certreq() polls to see
whether meanwhile the requested certificate is available.
In case the caller decides to abort the pending certificate request and provides
...
via B<OSSL_CMP_CTX_get0_newCert()>, the checkAfter value (which is > 0)
in case a 'waiting' status has been received (then B<OSSL_CMP_CTX_get0_newCert()>
yields NULL),
1 on successfully aborting a pending certificate request, and 0 on error.
