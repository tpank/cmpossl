...
CMP_CTX_set1_expected_sender() sets the distinguished name expected to be the
used by the sender response for messages proteced with MSG_SIG_ALG. During
validation of received messages, it gets enforced that this DN matches the
server name field of the PKIMessage header, which in turn is used to find the
server certificate.
This can be used to ensure that only particular entities are accepted to act as
...
If applicable, this defaults to the subject DN of the certificate set via
CMP_CTX_set1_srvCert().
