...
=head1 NAME
 ...
 CMP_CTX_set1_transfer_cb_opts,
 CMP_CTX_get_transfer_cb_opts,
...
CMP_CTX_set1_pkey() is the same as CMP_CTX_set0_pkey(), except that it does not
consume the pointer.
...
CMP_CTX_set1_newPkey() is the same as CMP_CTX_set0_newPkey(), except that it
...
The transfer callback may make use of proprietarily defined options stored in
the ctx by means of CMP_CTX_set1_transfer_cb_opts() and retrieved through
CMP_CTX_get_transfer_cb_opts().
On success it must return 0, else a CMP error reason code defined in cmp.h.

CMP_CTX_set1_transfer_cb_opts() sets an option, respecively structure
containing options, to be used by callback - if applicable.  B<opts> is not
consumed, and it must therefore explicitly be freed when needed. It may be NULL
to clear the entry.

CMP_CTX_get_transfer_cb_opts() gets the option, respecively structure
containing options, previously set with CMP_CTX_set1_transfer_cb_opts().
...
CMP_CTX_get_transfer_cb_opts() returns transfer callback options set previously.
