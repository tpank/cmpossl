...
=head1 SYNOPSIS
 ...
 int CMP_CTX_geninfo_itav_push0(CMP_CTX *ctx, CMP_ITAV *itav);
 int CMP_CTX_genm_itav_push0(CMP_CTX *ctx, CMP_ITAV *itav);
 ...
 typedef int (*cmp_transfer_cb_t)(const CMP_CTX *ctx, const CMP_MSG *req,
                                  CMP_MSG **res);
...
RFC 4210.
    ...
    CMP_CTX_OPT_POPOMETHOD
        Select the proof of possession method to use. Possible values are:
            ...
            CRMF_POPO_KEYENC - decrypt the encrypted certificate
                                 ("indirect method")
...
CMP_CTX_set1_caPubs() copies the given caPubs stack to the context.
...
CMP_CTX_set1_untrusted_certs() takes over a list of certificates containing
...
Default is CMP_MSG_http_perform().
...
CMP_CTX_set1_expected_sender() sets the Distinguished Name (DN) expected to be
given in the sender response for messages proteced with MSG_SIG_ALG. This may be
used to enforce that during validation of received messages the given DN matches
the sender field of the PKIMessage header, which in turn is used to identify the
server certificate.
...
example, the id-it-signKeyPairTypes OID and prints info on the genp contents.
    ...
    CMP_ITAV *itav = CMP_ITAV_new(type, NULL);
    CMP_CTX_genm_itav_push0(cmp_ctx, itav);

    STACK_OF(CMP_ITAV) *itavs;
    itavs = CMP_exec_GENM_ses(cmp_ctx);
    print_itavs(itavs);
    sk_CMP_ITAV_pop_free(itavs, CMP_ITAV_free);
