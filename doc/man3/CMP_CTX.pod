...
CMP_CTX_set1_last_snonce() stores the last sent sender B<nonce> in the B<ctx>.
...
CMP_CTX_set_msg_transfer() sets the message transfer callback function used for
...
int (*cmp_transfer_fn_t) (CMP_CTX *ctx, const CMP_PKIMESSAGE *req, CMP_PKIMESSAGE **res);
...
CMP_CTX_set_certConf_callback() sets a callback function that can be used to
check the newly enrolled certificate received from the server.
If may overrule the pre-decision reflected in 'failure' and '*txt' parameter.
...
given some set of trusted certificates, like with the -out-trusted CLI option.
It may also, for instance, compare the subject and other fields of the newly
enrolled cert with the requested certificate template.
