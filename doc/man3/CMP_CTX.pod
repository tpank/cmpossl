...
=head1 SYNOPSIS
 ...
 typedef int (*cmp_certConfFn_t) (CMP_CTX *ctx, const X509 *cert, int failure, const char **txt);
...
the CertTemplate structure when requesting a new cert.
If clCert is not set set (e.g. in case of IR with MSG_MAC_ALG), the subject name
...
CMP_CTX_set_error_callback() sets the callback for receiving error messages to
B<cb>.

CMP_CTX_set_debug_callback() sets a callback for receiving debug messages to
B<cb>.
...
If not set, the recipient name determined as specified above will be used.
...
CMP_CTX_set_msg_transfer() sets the message transfer callback function to B<cb>.
Default is CMP_PKIMESSAGE_http_perform().
This function sends the request PKIMessage and on success places the response in *res.
On success it must return 0, else a CMP error reason code defined in cmp.h.

CMP_CTX_set_certConf_callback() sets the callback for evaluating
evaluate the newly enrolled certificate received from the server to B<cb>.
The callback may overrule the pre-decision given in 'failure' and '*txt' parameter.
If it accepts the certificate the callback must return -1 (indicating success).
Else a bit number <= CMP_PKIFAILUREINFO_MAX indicates failure, and the txt
output parameter may be set to point to a constant string with more detail.
Typically, the callback will check at least that the certificate can be verified
using a set of trusted certificates.
It also might compare the subject and other fields of the newly
enrolled cert with the certificate template of the request.
...
name from the existing client certificate.  If a subject name is desired
...
CMP_CTX_error_callback() always returns 1.
