...
B<fail_info> is interpreted as bit pattern for the failInfo field.
...
OSSL_CMP_snprint_PKIStatusInfo() takes the PKIStatusInfo components contained
in the given statusInfo and places a human-readable string created from them
in the given buffer, with the given maximal length.
...
OSSL_CMP_CTX_snprint_PKIStatus()
returns a copy of the buffer pointer containing the string or NULL on error.
