...
overrule the pre-decision given in the B<fail_info> and B<*txt> parameters.  If
it accepts the certificate it must return -1, indicating success. Else it must
return an integer corresponding to a  bit field <= OSSL_CMP_PKIFAILUREINFO_MAX
indicating failure and may set the B<*txt> output parameter to point to a
string constant with more detail.  The transfer callback may make use of a
custom defined argument stored in the ctx by means of
OSSL_CMP_CTX_set_certConf_cb_arg(), which may be retrieved again through
OSSL_CMP_CTX_get_certConf_cb_arg().
...
OSSL_CMP_certConf_cb() example certConf callback validating that the new
certificate can be verified with set up trusted/untrusted certificates in
B<ctx>. As there is no requirement in the RFC that the certificate can be
validated by the client, this example is not set by default in the context.
Returns B<fail_info> if given !=0, 0 on successful validation, or else a
PKIFailureInfo bit field reflecting OSSL_CMP_PKIFAILUREINFO_incorrectData.
