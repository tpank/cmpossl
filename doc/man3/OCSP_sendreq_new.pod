...
OCSP_REQ_CTX_set1_req, OCSP_sendreq_bio, OCSP_query_responder
...
path B<path>, and the OCSP request B<req>.

OCSP_query_responder() performs an OCSP request using the responder BIO B<b>,
the optional host name B<host> to place in the HTTP request header, the URL
path B<path>, the optional list of further HTTP header entries B<headers>,
the OCSP request B<req>, and the optional timeout B<timeout> giving a positive
number of seconds or a value <= 0 for infinite.
...
OCSP_sendreq_bio() and OCSP_query_responder() return the B<OCSP_RESPONSE>
structure sent by the responder or B<NULL> if an error occurred.
...
OCSP_sendreq_bio() is retained for compatibility. It is implemented as

 OCSP_query_responder(b, NULL, path, NULL, req, -1)
