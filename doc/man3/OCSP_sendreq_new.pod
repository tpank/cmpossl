...
OCSP_sendreq_new, OCSP_sendreq, OSSL_HTTP_REQ_CTX_free,
OSSL_HTTP_REQ_CTX_set_max_response_length, OSSL_HTTP_REQ_CTX_add1_header,
...
OSSL_HTTP_REQ_CTX_free() frees up the OCSP context B<rctx>.
...
OSSL_HTTP_REQ_CTX_add1_header() adds header B<name> with value B<value> to the
...
function should be called after any calls to OSSL_HTTP_REQ_CTX_add1_header().
...
OCSP_sendreq_new() returns a valid B<OSSL_HTTP_REQ_CTX> structure or B<NULL>
if an error occurred.

OCSP_sendreq(), OSSL_HTTP_REQ_CTX_add1_header() and OCSP_REQ_CTX_set1_req()
...
The headers added with OSSL_HTTP_REQ_CTX_add1_header() are of the form
"B<name>: B<value>" or just "B<name>" if B<value> is B<NULL>. So to add
a Host header for B<ocsp.com> you would call:

 OSSL_HTTP_REQ_CTX_add1_header(ctx, "Host", "ocsp.com");
