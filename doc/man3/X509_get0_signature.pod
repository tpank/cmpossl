...
X509_CRL_get_signature_nid, X509_get_signature_info, OSSL__get_signature_info,
...
X509_get_signature_info() and OSSL_get_signature_info() retrieve information
about the signature of certificate B<x> or about the signature B<sig>
and related signature algorithm B<alg> of any signed object, respecitvely.
The NID of the signing digest is written to B<*mdnid>,
the public key algorithm to B<*pknid>, the effective security bits to
B<*secbits> and flag details to B<*flags>. Any of these parameters can
...
X509_get_signature_info() and OSSL_get_signature_info()
return 1 if the signature information
...
X509_SIG_INFO_set() and X509_SIG_INFO_init() do not return any values.
...
The X509_SIG_INFO_init() function was added in OpenSSL 3.0.
