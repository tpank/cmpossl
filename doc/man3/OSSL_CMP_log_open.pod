...
of logging may be required. Therefore the logging callback function has a
...
It is also preceded by any given severity indication, namely, "EMERGENCY",
...
unless data has been empty so far. Note that, in contrast,
L<ERR_add_error_data(3)> simply overwrites previous contents of the data field.
...
C<log_fn> if given, else B<OSSL_CMP_puts()> for uniformity with CMP logging.
