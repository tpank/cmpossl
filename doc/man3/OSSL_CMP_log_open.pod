...
=head1 SYNOPSIS
 ...
 #define OSSL_trace_set_verbosity(category, level)
...
a generalized logging/error callback mirroring the severity levels of syslog.h,
...
of logging may be required. Therefore the logging facility has a verbosity level
and the callback function has a B<level> parameter indicating the severity
...
by default to B<stdout>. It fails if the integrated tracing is disabled or STDIO
is not available. It should be called at least once during application startup.
It may be called multiple times.

OSSL_CMP_log_close() should be called when all activities are finished to flush
any pending CMP-specific log output and deallocate related resources.
It may be called multiple times.

OSSL_CMP_alert() outputs a simple error message via the trace API.
...
similarly to B<ERR_print_errors_cb()>, but using the CMP log callback function
