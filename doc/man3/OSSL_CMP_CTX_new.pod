...
Returns an empty stack if no extraCerts have been received or NULL on error.
...
OSSL_CMP_CTX_get_status() returns the PKIStatus from the last CertRepMessage or
Revocation Response, -1 on error.

OSSL_CMP_CTX_get0_statusString() returns the statusString from the last
CertRepMessage or Revocation Response, NULL on error.

OSSL_CMP_CTX_get_failInfoCode() returns the failinfo error code bits in context
...
X.509 certificates received in the caPubs field of last received certificate
response message IP/CP/KUP which had caPubs set.
Returns an empty list if there were no caPubs and NULL on error.
