...
arguments as with printf. In debug mode optional information in B<func>,
B<file>, and B<lineno> parameters is included. Output is sent to the
log callback if set in B<ctx>, else to L<OSSL_CMP_puts(3)>.
...
the log callback function if set in the B<ctx>, else L<OSSL_CMP_puts(3)>.
...
Requests (IR) and Certifiation Requests (CR) only if no SANs are set.
