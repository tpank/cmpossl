...
and any previous results (newClCert, caPubs, and extraCertsIn)
...
error/warning/info/debug messages. For details see L<OSSL_CMP_log_open(3)>.

OSSL_CMP_print_errors() outputs any entries in the OpenSSL error queue. It uses
the log callback function if set in the B<ctx>, else L<ERR_print_errors(3)>.
...
OSSL_CMP_CTX_geninfo_push0_ITAV() adds B<itav> to the stack in the B<ctx> to be
...
OSSL_CMP_CTX_push1_extraCertsOut() adds the given certificate to the outbound
...
OSSL_CMP_CTX_get0_newPkey() gets the newPkey from the context, or NULL if unset.
...
see B<OSSL_CMP_CTX_set1_oldClCert()>. This default is used for Initialization
...
OSSL_CMP_CTX_set1_oldClCert() sets the old certificate to be updated in
...
OSSL_CMP_CTX_genm_push0_ITAV() adds B<itav> to the stack in the B<ctx> which
...
OSSL_CMP_CTX_get_status() returns the PKIstatus from the last
...
OSSL_CMP_CTX_get_failInfoCode() returns the CMP error code from the failInfo
field of the last CertRepMessage or Revocation Response or error message.
This is a bit field and the flags for it are specified in
cmp.h. The flags start with OSSL_CMP_CTX_FAILINFO, for example:
OSSL_CMP_CTX_FAILINFO_badAlg. Returns -1 if the failInfo field is unset.

OSSL_CMP_CTX_get0_newClCert() returns the pointer to the newly obtained
...
OSSL_CMP_CTX_reinit() prepares the given B<ctx> for a further transaction by
clearing the session status (transactionID, last_senderNonce, and recipNonce)
and any previous results (failInfoCode, statusString, newClCert, caPubs, and
extraCertsIn) from the last executed transaction.
All other field values (e.g., CMP options) are retained for potential re-use.
