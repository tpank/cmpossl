...
The callback may modify the BIO B<bio> given by OSSL_CMP_MSG_http_perform(),
...
the callback function is invoked with the B<detail> argument being 1.
...
The callback should send the CMP request messaage it obtains via the B<req> 
