...
=head1 SYNOPSIS
  ...
  OSSL_CRMF_PBMPARAMETER *OSSL_CRMF_pbmp_new(size_t slen, int owfnid, long itercnt,
                                             int macnid);

=head1 DESCRIPTION

OSSL_CRMF_passwordBasedMac_new() generates a PBM based on the given OSSL_CRMF_PBMPARAMETER.
...
respective given parameters.  In the OSSL_CRMF_PBMPARAMETER structure, the NIDs for
...
OSSL_CRMF_pbmp_new() initializes and returns a new OSSL_CRMF_PBMPARAMETER structure. Returns
