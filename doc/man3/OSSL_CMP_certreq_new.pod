...
B<si> and CertID set to B<cid>. Consumes B<cid>. Accepts unprottected errors if
B<uprot_err> != 0;

OSSL_CMP_certConf_new() creates a Certificate Confirmation message for the last
received certificate. PKIStatus defaults to accepted. If the B<fail_info> bit
field is set to !=0, it is set to the failInfo of the PKIStatusInfo, and status
is set to rejected.  If B<text> is not NULL, its content is copied to
statusString.
...
OSSL_CMP_certConf_new() could be enhanced to set more than one failInfo bit.
