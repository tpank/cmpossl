...
received certificate. PKIStatus defaults to accepted. If B<failure> is set to
!=0, it sets status to rejected, sets the failInfo bit with index
B<failure>-1, and copies content of B<text> to statusString if not NULL.
