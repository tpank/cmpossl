...
=head1 NAME
  ...
  CMP_PKISTATUSINFO_PKIFailureInfo_get,
  CMP_PKISTATUSINFO_PKIFailureInfo_check,
...
=head1 SYNOPSIS
  ...
  int CMP_PKISTATUSINFO_PKIFailureInfo_check( CMP_PKISTATUSINFO *si, int codeBit);
...
CMP_PKISTATUSINFO_PKIFailureInfo_get() returns the PKIFailureInfo bits
of a given PKIStatusInfo, encoded as long integer, or -1 on error.

CMP_PKISTATUSINFO_PKIFailureInfo_check() checks whether a given bit is set
