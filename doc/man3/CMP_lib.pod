...
=head1 NAME
  ...
  CMP_PKISI_PKIFailureInfo_get,
  CMP_PKISI_PKIFailureInfo_check,
  CMP_HDR_generalInfo_item_push0,
  CMP_MSG_generalInfo_items_push1,
  CMP_HDR_new_senderNonce,
  CMP_HDR_push0_freeText,
  CMP_HDR_push1_freeText,
  CMP_PKIFREETEXT_push_str,
  CMP_MSG_get0_header,
  CMP_HDR_get0_transactionID,
  CMP_HDR_get0_senderNonce,
  CMP_HDR_get0_recipNonce,
  CMP_HDR_init,
  CMP_HDR_set1_recipient,
  CMP_HDR_set1_sender,
  CMP_HDR_set1_senderKID,
  CMP_HDR_set1_transactionID,
  CMP_HDR_set_messageTime,
  CMP_HDR_set_version,
  CMP_MSG_check_implicitConfirm,
  CMP_MSG_genm_item_push0,
  CMP_MSG_get_bodytype,
  CMP_MSG_get_transactionID,
  CMP_PKISI_snprint,
  CMP_MSG_set_bodytype,
  CMP_MSG_set_implicitConfirm,
  OSSL_CMP_calc_protection,
  CMP_MSG_protect,
  CMP_MSG_add_extraCerts,
  CMP_ITAV_new,
  CMP_statusInfo_new,
  CMP_PKISI_PKIStatus_get,
  CMP_get_protectionAlg_pbmac,
  CMP_CERTRESPONSE_get_certificate,
  CMP_build_cert_chain,
  CMP_MSG_check_received,
...
=head1 SYNOPSIS
  ...
  CMP_PKISI *CMP_REVREPCONTENT_PKIStatusInfo_get(CMP_REVREPCONTENT *rrep, long rsid);
  ...
  long CMP_PKISI_PKIFailureInfo_get( CMP_PKISI *si);
  int CMP_PKISI_PKIFailureInfo_check( CMP_PKISI *si, int codeBit);
  int CMP_HDR_generalInfo_item_push0(CMP_HDR *hdr, const CMP_ITAV *itav);
  int CMP_MSG_generalInfo_items_push1(CMP_MSG *msg, STACK_OF(CMP_ITAV) *itavs);
  int CMP_HDR_new_senderNonce(CMP_HDR *hdr);
  int CMP_HDR_push0_freeText( CMP_HDR *hdr, ASN1_UTF8STRING *text);
  int CMP_HDR_push1_freeText( CMP_HDR *hdr, ASN1_UTF8STRING *text);
  CMP_PKIFREETEXT *CMP_PKIFREETEXT_push_str(CMP_PKIFREETEXT *ft, const char *text);
  CMP_HDR *CMP_MSG_get0_header(const CMP_MSG *msg);
  ASN1_OCTET_STRING *CMP_HDR_get0_transactionID(const CMP_HDR *hdr);
  ASN1_OCTET_STRING *CMP_HDR_get0_senderNonce(const CMP_HDR *hdr);
  ASN1_OCTET_STRING *CMP_HDR_get0_recipNonce(const CMP_HDR *hdr);
  int CMP_HDR_init(CMP_CTX *ctx, CMP_HDR *hdr);
  int CMP_HDR_set1_recipient(CMP_HDR *hdr, const X509_NAME *nm);
  int CMP_HDR_set1_sender(CMP_HDR *hdr, const X509_NAME *nm);
  int CMP_HDR_set1_senderKID(CMP_HDR *hdr, const ASN1_OCTET_STRING *senderKID);
  int CMP_HDR_set1_transactionID(CMP_HDR *hdr, const ASN1_OCTET_STRING *transactionID);
  int CMP_HDR_set_messageTime(CMP_HDR *hdr);
  int CMP_HDR_set_version(CMP_HDR *hdr, int version);
  int CMP_MSG_check_implicitConfirm(CMP_MSG *msg);
  int CMP_MSG_genm_item_push0(CMP_MSG *msg, const CMP_ITAV *itav);
  int CMP_MSG_genm_items_push1(CMP_MSG *msg, STACK_OF(CMP_ITAV) *itavs);
  int CMP_MSG_get_bodytype(const CMP_MSG *msg);
  ASN1_OCTET_STRING *CMP_MSG_get_transactionID(const CMP_MSG *msg);
  char *CMP_PKISI_snprint(CMP_PKISI *si, char *buf, int bufsize);
  int CMP_MSG_set_bodytype( CMP_MSG *msg, int type);
  int CMP_MSG_set_implicitConfirm(CMP_MSG *msg);
  ASN1_BIT_STRING *OSSL_CMP_calc_protection(const CMP_MSG *msg,
                                       const ASN1_OCTET_STRING *secret,
                                       const EVP_PKEY *pkey);
  int CMP_MSG_protect(CMP_CTX *ctx, CMP_MSG *msg);
  int CMP_MSG_add_extraCerts(CMP_CTX *ctx, CMP_MSG *msg);
  CMP_ITAV *CMP_ITAV_new(const ASN1_OBJECT *type, const ASN1_TYPE *value);
  CMP_PKISI *CMP_statusInfo_new(int status, unsigned long failInfo,
                                        const char *text);
  long CMP_PKISI_PKIStatus_get( CMP_PKISI *statusInfo);
  ...
  int CMP_MSG_check_received(CMP_CTX *ctx, const CMP_MSG *prev,
                                    const CMP_MSG *msg, int callback_arg,
...
CMP_PKISI_PKIFailureInfo_get() returns the PKIFailureInfo bits
of a given PKIStatusInfo, encoded as long integer, or -1 on error.

CMP_PKISI_PKIFailureInfo_check() checks whether a given bit is set
within the PKIFailureInfo of a given PKIStatusInfo. Returns -1 on error.

CMP_HDR_generalInfo_item_push0() adds the given InfoTypeAndValue
item to the hdr->generalInfo stack. Consumes the B<itav> pointer.

CMP_MSG_generalInfo_items_push1() adds a copy of the B<itavs> stack to
the generalInfo field of PKIheader of the B<msg>. Does not consume the B<itavs>
pointer.

CMP_HDR_new_senderNonce() generates a random value for the
hdr->senderNonce field, which is used to protect the PKIMessage against
replay attacks.

CMP_HDR_push0_freeText() pushes an ASN1_UTF8STRING to
hdr->freeText and consumes the given pointer.

CMP_HDR_push1_freeText() pushes an ASN1_UTF8STRING to
...
CMP_MSG_get0_header returns the header of the given CMP message.

CMP_HDR_get0_transactionID returns the transaction ID of the given PKIHeader.

CMP_HDR_get0_senderNonce returns the sender nonce of the given PKIHeader.

CMP_HDR_get0_recipNonce returns the recipient nonce of the given PKIHeader.

CMP_HDR_init() initializes a PKIHeader structure based on the
values in the given CMP_CTX structure.

CMP_HDR_set1_recipient() sets the recipient field in the given
PKIHeader to the given X509 Name value, without consuming the pointer.

CMP_HDR_set1_sender() sets the sender field in the given PKIHeader
to the given X509 Name value, without consuming the pointer.

CMP_HDR_set1_senderKID() Sets hdr->senderKID to the given
...
CMP_HDR_set1_transactionID() copies the given transactionID to
hdr->transactionID. If the transactionID parameter is NULL, a new random
value is generated for it.

CMP_HDR_set_messageTime() sets the messageTime to the current system time.

CMP_HDR_set_version() sets hdr->pvno to the given integer.

CMP_MSG_check_implicitConfirm() returns 1 if implicitConfirm is
set in the given PKIMessage, 0 if not.

CMP_MSG_genm_item_push0() pushes the B<itav> to the body of the
of the PKIMessage B<msg> of GenMsg type. Consumes the B<itavs> pointer.

CMP_MSG_genm_items_push1() adds a copy of the B<itavs> stack to the body
of the PKIMessage B<msg> of GenMsg type. Does not consume the B<itavs> pointer.

CMP_MSG_get_bodytype() returns the body type of the given
PKIMessage.

CMP_MSG_get_transactionID() returns the transactionID of the given PKIMessage.

ASN1_OCTET_STRING *CMP_MSG_get_transactionID(const CMP_MSG *msg)

CMP_PKISI_snprint() places human-readable error string
created from PKIStatusInfo in given buffer.

CMP_MSG_set_bodytype() sets the type of the message contained in
the PKIMessage body field.

CMP_MSG_set_implicitConfirm() sets implicitConfirm in the
...
CMP_MSG_protect() protects the given message deciding on the algorithm
...
CMP_MSG_add_extraCerts() fills the extraCerts field in the message.

CMP_ITAV_new() creates a new CMP_ITAV structure and fills it in.

CMP_statusInfo_new() creates a new PKIStatusInfo structure and fills it in.

CMP_PKISI_PKIStatus_get() returns the status value of the given PKIStatusInfo.
...
CMP_MSG_check_received() checks for the given message received (i.e.,
