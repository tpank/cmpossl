...
It the typical generic checks on the given request message, calls
...
OSSL_CMP_server_perform() is an interface to B<OSSL_CMP_SRV_process_request()>
that can be used by a CMP client like B<OSSL_CMP_MSG_http_perform()>.
