...
OSSL_CMP_server_perform() implements a generic CMP server.
It is used similarly to B<OSSL_CMP_MSG_http_perform()>.
A B<OSSL_CMP_SRV_CTX> must be set as B<transfer_cb_arg>.
The funcion does the typical generic checks on incoming messages,
then calls the appropriate callback function for more specific processing,
and then assembles a result message (unless an internal error occurred).
