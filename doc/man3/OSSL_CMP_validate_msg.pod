...
is preferrably the one provided by a call to L<OSSL_CMP_CTX_set1_srvCert(3)>.
If this not set then candidate sender certificates are tried from
the list of certificates received in the C<msg> extraCerts, then any
...
where a candidate is only accepable if its subject matches the C<msg> sender DN,
its subject key identifier matches the senderKID (as far as present), and it can
be verified using any trust store set via L<OSSL_CMP_CTX_set0_trustedStore(3)>.
Any cert that has been successfully used to check a message signatures is cached
such that subsequent messages in the same transaction can be verified quicker.
