...
=head1 SYNOPSIS
  ...
  int CMP_MSG_protect(OSSL_CMP_CTX *ctx, OSSL_CMP_MSG *msg);
  int CMP_MSG_add_extraCerts(OSSL_CMP_CTX *ctx, OSSL_CMP_MSG *msg);

=head1 DESCRIPTION

CMP_MSG_protect() protects the given message B<msg> using an algorithm
...
it selects Signature and uses B<CMP_MSG_add_extraCerts()>.
It also sets the protectionAlg field in the message header accordingly.

CMP_MSG_add_extraCerts() adds elements to the extraCerts field in the given
