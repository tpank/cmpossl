...
# define OSSL_CMP_PKIBODY_POLLREP  26
 ...
 OSSL_CMP_MSG *CMP_certreq_new(OSSL_CMP_CTX *ctx, int bodytype, int err_code);
 ...
 OSSL_CMP_MSG *CMP_pollRep_new(OSSL_CMP_CTX *ctx, int crid, int poll_after)
...
CMP_certreq_new() creates a PKIMessage for requesting a certificate,
...
CMP_certrep_new() creates a PKIMessage for certificate response, which can
...
CMP_rr_new() creates a Revocation Request message from the
information set via OSSL_CMP_CTX_set1_oldClCert().

CMP_rp_new() creates a Revocation Response message with status set to
B<si> and CertID set to B<cid>. Consumes B<cid>.
Accepts unprotected errors if B<uprot_err> != 0.

CMP_certConf_new() creates a Certificate Confirmation message for the last
...
CMP_pkiconf_new() creates a PKI Confirmation message.

CMP_pollReq_new() creates a Polling Request message with certReqId set to
B<crid>.

CMP_pollRep_new() creates a Polling Response message with certReqId set to
B<crid> and pollAfter to B<poll_after>.

CMP_genm_new() creates a new General Message with an empty ITAV stack.

CMP_genp_new() creates a new General Response with an empty ITAV stack.

CMP_error_new() creates a new Error Message with the given contents,
