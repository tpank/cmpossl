...
static int setup_request_ctx(OSSL_CMP_CTX *ctx, ENGINE *engine)
{
    ...
    if (opt_csr != NULL) {
        if (opt_cmd == CMP_GENM) {
            ...
            X509_REQ *csr = load_csr_autofmt(opt_csr, "PKCS#10 CSR");
    ...
    if (opt_port != NULL) { /* act as very basic CMP HTTP server */
        ...
        next:
            if (!ret) /* on transmission error, cancel CMP transaction */
                OSSL_CMP_CTX_set1_transactionID(srv_cmp_ctx, NULL);
# if 1 /* keep-alive */    
            if (!ret || OSSL_CMP_CTX_get_status(srv_cmp_ctx) == -1) {
                BIO_free_all(cbio);
                cbio = NULL;
            }
# else
