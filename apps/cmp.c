...
# if !defined(OPENSSL_NO_SOCK)
        x = X509_load_http(file, NULL, NULL, opt_crl_timeout);
# else
...
# else
        CMP_err1("http(s) not supported loading CRL from '%s'\n", infile);
# endif
        goto end;
    }
    if (can_retry)
...
static STACK_OF(X509_CRL) *LOCAL_crls_http_cb(const X509_STORE_CTX *ctx,
                                              const X509_NAME *nm)
{
    X509 *x;
    STACK_OF(X509_CRL) *crls = NULL;
...
static STACK_OF(X509_CRL) *get_crls_cb(const X509_STORE_CTX *ctx, const X509_NAME *nm)
{
...
static int check_cert_revocation(X509_STORE_CTX *ctx, OCSP_RESPONSE *resp)
{
    ...
    if (ocsp_stapling) {
        ...
        if (ok == 0
            /*
             * cert revoked, thus clear failure or
...
static int ocsp_stapling_cb(SSL *ssl, STACK_OF(X509) *untrusted)
{
    ...
    int ret = -1; /*
                   * tls_process_initial_server_flight reports
...
# undef FORMAT_OPTIONS
    ...
    if (opt_ownform_s != NULL
            && !opt_format(opt_ownform_s, OPT_FMT_PEMDER | OPT_FMT_PKCS12,
                           &opt_ownform)) {
        CMP_err("unknown option given for own certificate format");
        return 0;
    }
    ...
    if (opt_otherform_s != NULL
            && !opt_format(opt_otherform_s, OPT_FMT_PEMDER | OPT_FMT_PKCS12,
                           &opt_otherform)) {
        CMP_err("unknown option given for certificate store format");
        return 0;
    }

    if (opt_crlform_s != NULL
            && !opt_format(opt_crlform_s, OPT_FMT_PEMDER, &opt_crlform)) {
        CMP_err("unknown option given for CRL format");
        return 0;
    }
...
# endif /* OPENSSL_NO_OCSP */
    if (opt_crls != NULL || opt_crl_download) {
        ...
        CMP_err("must use -crl_download or -crls when -crl_check is given");
        goto err;
    }
...
#  endif
        ...
        if (!check_revocation) {
            CMP_err("internal issue: cannot get check_revocation");
            goto err;
        }
...
static int setup_protection_ctx(OSSL_CMP_CTX *ctx, ENGINE *e)
{
    ...
    if (opt_key != NULL) {
        EVP_PKEY *pkey = load_privkey_autofmt(opt_key, opt_keyform, opt_keypass,
    ...
    if (opt_cert != NULL) {
        ...
        if (!load_certs_autofmt(opt_cert, &certs, opt_ownform, 1, opt_keypass,
