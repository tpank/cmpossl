...
int ocsp_main(int argc, char **argv)
{
 ...
 opthelp:
        ...
        case OPT_PATH:
            ...
            break;
        case OPT_ISSUER:
            issuer = load_cert(opt_arg(), FORMAT_PEM, NULL,
                               "issuer certificate");
            if (issuer == NULL)
                goto end;
            if (issuers == NULL) {
        ...
        case OPT_CERT:
            X509_free(cert);
            cert = load_cert(opt_arg(), FORMAT_PEM, NULL, "certificate");
            if (cert == NULL)
                goto end;
            if (cert_id_md == NULL)
                cert_id_md = EVP_sha1();
    ...
    if (rsignfile != NULL) {
        if (rkeyfile == NULL)
            rkeyfile = rsignfile;
        rsigner = load_cert(rsignfile, FORMAT_PEM, NULL,
...
redo_accept:
    ...
    if (signfile != NULL) {
        if (keyfile == NULL)
            keyfile = signfile;
        signer = load_cert(signfile, FORMAT_PEM, NULL, "signer certificate");
        if (signer == NULL) {
