...
system ("export LD_LIBRARY_PATH=../../;../../apps/openssl cmp -port " .
    $localport . " -srv_cert " . catfile($datadir, "server.crt") .
    " -srv_key " . catfile($datadir, "server.key") .
    " -accept_unprotected -rsp_cert " . catfile($datadir, "client.crt") .
    " -certout /tmp/newcert.crt &");

ok(run(app(["openssl", "cmp", "-server", "localhost:" . $localport,
    "-cmd", "ir", "-cert", catfile($datadir, "client.crt"),
    "-key", catfile($datadir, "client.key"),
    "-certout", "/tmp/newcert.crt", "-trusted", catfile($datadir, "server.crt"),
    "-unprotectedrequests", "-unprotectederrors", "-no_check_time"])));	
    
# cleanup
system ("killall openssl");
